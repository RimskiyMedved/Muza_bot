...
from config import (
    ...
    HEALTHCHECK_URL, HEALTHCHECK_PING_INTERVAL, HEARTBEAT_HOUR,
)
from utils import (
    MONTHS,
    MONTHS_RU,
...
async def _notify_admin(application, text: str) -> None:
    """Шлёт сообщение суперадмину, молча проглатывая ошибки доставки."""
    try:
        await application.bot.send_message(SUPERADMIN_ID, text, parse_mode="HTML")
    except Exception as e:
...
async def on_bot_error(update, context) -> None:
    ...
    err = "".join(
        _traceback.format_exception_only(type(context.error), context.error)
    ).strip()
    await _notify_admin(
        context.application,
...
async def _healthcheck_ping_loop(application) -> None:
    """Пока бот жив — пингует Healthchecks.io. Прекращение пингов = тревога от сервиса."""
    if not HEALTHCHECK_URL:
        log.info("  Healthcheck    : ⏭  HEALTHCHECK_URL не задан, пропускаем")
        return
    log.info("  Healthcheck    : ✅ пинг каждые %d с", HEALTHCHECK_PING_INTERVAL)
    while True:
        try:
...
async def _daily_heartbeat_loop(application) -> None:
    """Раз в сутки в HEARTBEAT_HOUR шлёт админу отчёт «бот работает»."""
    while True:
        now = datetime.now()
        ...
        try:
            ...
            txt = (
                "✅ <b>Бот работает</b>\n"
                f"Броней всего: {s.get('bookings_total', '—')}\n"
                f"Предстоящих: {s.get('bookings_future', '—')}\n"
                f"Лидов: {s.get('leads_total', '—')}"
            )
        except Exception as e:
            txt = (
                "✅ <b>Бот работает</b>\n"
...
async def _heartbeat_write_loop(application) -> None:
    """Раз в минуту пишет отметку живости в БД — её читает /health для внешнего мониторинга."""
    while True:
        try:
            database.set_bot_heartbeat()
        except Exception as e:
...
def main() -> None:
    ...
    async def post_init(application) -> None:
        ...
        # ── Мониторинг: старт-уведомление + фоновые задачи ────────────────────
        await _notify_admin(application, "🟢 <b>Бот запущен</b>")
        asyncio.create_task(_heartbeat_write_loop(application))
        asyncio.create_task(_healthcheck_ping_loop(application))
        asyncio.create_task(_daily_heartbeat_loop(application))
