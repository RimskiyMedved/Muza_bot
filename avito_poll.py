...
═══════════════════════════════════════════════════════════════
  ...
  — Если API не помечает чат как unread — догоняем раз в N циклов по известным чатам (AVITO_POLL_STALE_CHECK_EVERY)
...
async def _sync_missed_inbound_for_known_chats(
    ...
    """
    Для чатов из _greeted_chats сравниваем последнее входящее с _last_handled.
    Если клиент написал новое, а чат не попал в unread_only — догоняем через _process_chat.
    """
    # Чаты, где уже был диалог по версии бота (приветствие или любое обработанное сообщение)
    candidates = set(_greeted_chats) | set(_last_handled.keys())
    if not candidates:
        return

    caught = 0
    for chat_id in list(candidates):
        if not chat_id:
            continue
        try:
            messages = await client.get_messages(chat_id, limit=20)
        except Exception as e:
            log.warning("[%s] stale-check get_messages %s: %s", account_name, chat_id[:16], e)
            continue
        ...
        log.info(
            "[%s] 🔄 догоняем без unread: chat=%s… последний in_msg=%s… (в состоянии было %s)",
            account_name,
            chat_id[:16],
            mid[:24],
        ...
        try:
            ...
            log.error(
                "[%s] stale-check _process_chat %s: %s",
                account_name, chat_id[:16], e,
                exc_info=True,
            )

    if caught:
        log.info("[%s] stale-check: догнано чатов с новым сообщением: %d", account_name, caught)
...
async def avito_polling_loop(client: AvitoClient, application) -> None:
    ...
    log.info(
        "[%s] 🚀 Поллер запущен (интервал %d сек, stale-check каждые %s циклов)",
        client.name,
        POLL_INTERVAL,
        STALE_CHECK_EVERY if STALE_CHECK_EVERY else "— откл.",
    )
    if not AVITO_NOTIFY_GROUP:
