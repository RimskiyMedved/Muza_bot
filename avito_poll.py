...
REPLY_FREE = (
    ...
    "менеджер свяжется и пришлёт презентацию с условиями."
)

REPLY_BUSY = "Эта дата, к сожалению, уже занята. Давайте рассмотрим другие даты?"

REPLY_THANKS = "Спасибо! 🤍"


REPLY_FAREWELL = (
    "Понимаем! Если появятся подходящие даты или захотите вернуться — "
    "мы всегда рады помочь. Всего доброго! 🤍"
)
...
REPLY_ASK_PHONE = (
    ...
    "Пришлите, пожалуйста, номер телефона — свяжемся и всё расскажем!"
)
...
_offered_alternatives: set[str]        = set()   # предложили альтернативы после занятой даты
_faq_count:            dict[str, int]  = {}      # chat_id → кол-во FAQ-ответов пока ждём телефон
_stuck_notified:       set[str]        = set()   # чаты где уже уведомили менеджера о тупике
_ignored_chats:        set[str]        = set()   # чаты продавцов/спама — больше не отвечаем
...
def _load_state() -> None:
    ...
    try:
        ...
        _offered_alternatives = set(data.get("offered_alternatives", []))
...
_PRICE_KEYWORDS    = {"какая цена","сколько стоит","прайс",
                      "расценки","почём","почем","аренда стоит",
                      "цена аренды","стоимость аренды","сколько стоит аренда","условия аренды",
...
# Продавец / спам — человек пытается что-то продать, а не арендовать зал
_SELLER_KEYWORDS = {
    "продаю", "продам", "продаётся", "продается", "на продажу",
    ...
    "услугах профессиональной", "услуги профессиональной",
    "профессиональные услуги", "предлагаем услуги",
}

# Клиент отказывается от предложенных альтернатив (используется только когда _offered_alternatives)
_DECLINE_KEYWORDS = {
    "не интересно", "неинтересно", "не интересует", "не интересуют",
    "не интересна", "не подходит", "не подходят", "ничего не подходит",
    "не буду", "передумал", "передумала", "не нужно", "не надо",
...
def _is_seller(text: str) -> bool:
    """Человек пытается что-то продать, а не арендовать зал."""
    t = text.lower()
    return any(kw in t for kw in _SELLER_KEYWORDS)
...
def _find_nearby_free_dates(
    target: date, window: int = 7
) -> tuple["date | None", "date | None"]:
    """
    Ищет ближайшую свободную дату ДО и ПОСЛЕ target в пределах window дней.
    «Свободная» = не занята в таблице бронирований (check_date.found == False).
    Даты в прошлом не предлагаются.
    Возвращает (дата_до, дата_после).
    """
    ...
    for delta in range(1, window + 1):
        if before is None:
            ...
            if candidate >= today:
                try:
                    if not check_date(candidate)["found"]:
                        before = candidate
                except Exception:
                    pass
...
def _format_client_msg(
    ...
    *,
    ...
    # ── История последних 8 сообщений — ниже, от предпоследнего к старым ────
    if all_messages:
        history = [
        ...
        history = history[-8:]
        history.reverse()  # предпоследнее сверху, самое старое снизу

        if history:
            lines.append("")
            for m in history:
                ts  = _fmt_ts(m.get("created", 0))
                out = (m.get("direction") or "").lower() == "out"
...
async def _process_chat(
    ...
    *,
    ...
    ctx = _chat_context.setdefault(chat_id, {"date": None, "guests": "", "event": "", "phone": ""})
    if not ctx["date"]:
    ...
    if _is_seller(msg_text):
        ...
        try:
            await client.send_message(
                chat_id,
                "Спасибо за предложение! К сожалению, мы не занимаемся закупкой — "
                "мы сами являемся площадкой для мероприятий. Удачи вам! 🤍",
            )
        except Exception as e:
            log.warning("   ↳ Не удалось отправить отказ продавцу: %s", e)
        try:
            await client.mark_read(chat_id)
        except Exception:
            pass
    ...
    # ── Вспомогательная: тегнуть менеджера когда есть и контакт и свободная дата
    async def _notify_manager_ready(contact: str, d: "date | None") -> None:
        if not (AVITO_NOTIFY_GROUP and ADMIN_TG_USERNAME):
            return
        date_line = f"📅 Дата: {d.strftime('%d.%m.%Y')}\n" if d else ""
        try:
            await application.bot.send_message(
                chat_id=AVITO_NOTIFY_GROUP,
                text=(
                    f"🎯 {ADMIN_TG_USERNAME}, клиент готов к бронированию!\n"
                    f"👤 {_e(buyer_name or 'Клиент')}\n"
                    f"📱 {_e(contact)}\n"
                    f"{date_line}"
                    f"✅ Дата свободна — можно звонить!"
                ),
                parse_mode="HTML",
            )
            log.info("   ↳ Менеджер тегнут (контакт + свободная дата)")
        except Exception as e:
            log.warning("   ↳ Не удалось тегнуть менеджера: %s", e)

    # ── Вспомогательная: сохранить контакт и уведомить ───────────────────────
    async def _save_contact(phone: str, nick: str, skip_date_ask: bool = False) -> None:
        ...
        _faq_count.pop(chat_id, None)      # сброс счётчика FAQ-напоминаний
        _stuck_notified.discard(chat_id)   # разблокируем уведомление если позвонят снова
        contact_line = phone or nick

        # Уведомление о новом лиде (всегда, даже без даты)
        if AVITO_NOTIFY_GROUP and ADMIN_TG_USERNAME:
            try:
                ...
                log.info("   ↳ Уведомление о новом лиде отправлено")
            except Exception as e:
                log.warning("   ↳ Не удалось уведомить о лиде: %s", e)
        ...
        if known_date:
            ...
            if skip_date_ask:
                ...
                log.info("   ↳ даты нет — спрашиваем")
                auto_replies.append(
                    "Спасибо! 🤍 Скажите, пожалуйста, какую дату мероприятия рассматриваете? "
                    "Проверим, свободна ли она! 🗓"
                )

    # ── Вспомогательная: проверка дат ────────────────────────────────────────
    async def _check_dates_reply(dates: list) -> None:
        have_contact = chat_id in _lead_received  # проверяем в реальном времени
        ...
        if len(dates) == 1:
            ...
            try:
                ...
                if result["found"]:
                    log.info("   ↳ ЗАНЯТО")
                    nb, na = _find_nearby_free_dates(d)
                    if nb or na:
                        alts = []
                        if nb:
                            alts.append(f"• {nb.strftime('%d.%m.%Y')} (за {(d - nb).days} дн.)")
                        if na:
                            alts.append(f"• {na.strftime('%d.%m.%Y')} (+{(na - d).days} дн.)")
                        reply = (
                            "Эта дата, к сожалению, уже занята.\n\n"
                            "Ближайшие свободные:\n" + "\n".join(alts) +
                            "\n\nКакая из них подойдёт?"
                        )
                    ...
                    log.info("   ↳ СВОБОДНО")
                    _offered_alternatives.discard(chat_id)
                    if have_contact:
                        ...
                        contact_line = ctx.get("phone", "")  # берём из ctx если есть
                        await _notify_manager_ready(contact_line or buyer_name or "?", d)
                    else:
                        # Контакта ещё нет — просим телефон
                        auto_replies.append(REPLY_FREE)
                        _awaiting_contact.add(chat_id)
            except Exception as e:
                log.error("   ↳ check_date: %s", e, exc_info=True)
                log.warning("   ↳ ответ не отправлен — ошибка check_date")
        else:
            # Несколько дат
            lines = []
            any_free = False
            ...
            if lines:
                ...
                if any_free:
                    if have_contact:
                        ...
                        reply += "\n\nПришлите, пожалуйста, номер телефона — менеджер свяжется и пришлёт условия."
                        _awaiting_contact.add(chat_id)
                else:
    ...
    if is_first:
        ...
        # Отвечаем на все FAQ-вопросы из первого сообщения (if, не elif — одно сообщение
        # может содержать сразу несколько вопросов). Приветствие уже спросило дату.
        if _is_amenities_question(msg_text):
            log.info("   ↳ Первое сообщение: FAQ мебель/декор")
            auto_replies.append(FAQ_AMENITIES)
        if _is_food_question(msg_text):
            log.info("   ↳ Первое сообщение: FAQ еда/алкоголь")
        ...
        # Если в первом сообщении уже есть контакт — сохраняем лид сразу.
        # skip_date_ask=True: приветствие уже спросило дату, не дублируем.
        if _has_phone(msg_text) or _has_tg_nick(msg_text):
            log.info("   ↳ Первое сообщение содержит контакт — сохраняем лид")
            phone = _extract_phone(msg_text) if _has_phone(msg_text) else ""
            nick  = _extract_tg_nick(msg_text) if _has_tg_nick(msg_text) else ""
            await _save_contact(phone, nick, skip_date_ask=True)

    # ── Клиент отказывается от предложенных альтернатив → прощаемся ─────────
    elif chat_id in _offered_alternatives and _is_declining(msg_text):
        log.info("   ↳ Отказ от альтернативных дат — прощаемся")
        auto_replies.append(REPLY_FAREWELL)
        _offered_alternatives.discard(chat_id)
    ...
    # ── Обычные сообщения: FAQ + контакт + дата параллельно ──────────────────
    #
    #  Используем if (не elif), чтобы одно сообщение могло:
    ...
    #   → ответить на FAQ + сохранить контакт + проверить дату
        ...
        #   Приоритет CTA:
        #     1. Дата свободна, ждём телефон → просим телефон (не чаще 2 раз подряд)
        #     2. Предложены альтернативы → уточняем по альтернативам (не спрашиваем дату заново)
        #     3. Даты нет вообще → просим дату
        #     4. Всё есть (или лид получен) → чистый FAQ без CTA

        def _faq_cta() -> tuple[str, bool]:
            """Возвращает (суффикс CTA, флаг asked_phone)."""
            if awaiting:
                n = _faq_count.get(chat_id, 0)
                _faq_count[chat_id] = n + 1
                if n < 2:
                    return (
                        "\n\nПришлите, пожалуйста, номер телефона — менеджер свяжется с вами. 🤍",
                        True,
                    )
                return ("", False)   # ≥ 3-й вопрос подряд — не давим
            if chat_id in _offered_alternatives:
                return (
                    "\n\nКстати, по датам — вас устраивают предложенные варианты?",
                    False,
                )
            if not ctx.get("date"):
                return (
        ...
        if _is_amenities_question(msg_text):
            ...
            auto_replies.append(FAQ_AMENITIES + cta)
            if asked_ph:
        ...
        if _is_food_question(msg_text):
            ...
            cta, asked_ph = _faq_cta()
            auto_replies.append(FAQ_FOOD + cta)
            if asked_ph:
                faq_asked_phone = True
            faq_answered = True
        ...
        if _is_capacity_question(msg_text):
            ...
            cta, asked_ph = _faq_cta()
            auto_replies.append(FAQ_CAPACITY + cta)
            if asked_ph:
                faq_asked_phone = True
            faq_answered = True

        # ── Шаг 2: Сохраняем контакт если есть (независимо от FAQ) ──────────
        has_contact_now = _has_phone(msg_text) or _has_tg_nick(msg_text)
        if has_contact_now:
            phone = _extract_phone(msg_text) if _has_phone(msg_text) else ""
            nick  = _extract_tg_nick(msg_text) if _has_tg_nick(msg_text) else ""
            await _save_contact(phone, nick)
            # _save_contact: сохраняет лид + благодарит + проверяет дату или просит её

        # ── Шаг 3: Проверяем даты (если контакт не в этом сообщении) ─────────
        if not has_contact_now:
            dates = _parse_all_dates(msg_text)
            if dates:
                _asked_date.discard(chat_id)
                _stuck_notified.discard(chat_id)   # клиент дал дату — тупик разрешён
                _offered_alternatives.discard(chat_id)
                log.info("   ↳ найдено дат: %d — проверяем", len(dates))
                await _check_dates_reply(dates)
            elif not faq_answered:
                ...
                if not msg_text.strip():
                    ...
                    log.info("   ↳ дату уже спрашивали — молчим, ждём менеджера")
                    # Уведомляем менеджера один раз что клиент завис
                    if chat_id not in _stuck_notified and AVITO_NOTIFY_GROUP and ADMIN_TG_USERNAME:
                        try:
                            await application.bot.send_message(
                                chat_id=AVITO_NOTIFY_GROUP,
                                text=(
                                    f"⚠️ {ADMIN_TG_USERNAME}, клиент не отвечает на вопрос о дате.\n"
                                    f"👤 {_e(buyer_name or 'Клиент')} — возможно, стоит подключиться вручную."
                                ),
                                parse_mode="HTML",
                            )
                            _stuck_notified.add(chat_id)
                            log.info("   ↳ Менеджер уведомлён о тупике")
                        except Exception as e:
                            log.warning("   ↳ Не удалось уведомить о тупике: %s", e)
    ...
    if (
        ...
        auto_replies.append(
            "Напомним: пришлите, пожалуйста, номер телефона — менеджер свяжется с вами. 🤍"
        )
